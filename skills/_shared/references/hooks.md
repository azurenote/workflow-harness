...
| `pre_commit` | `project-start` Step 7(구현 완료 후 포맷) · `project-done` Step 5(커밋) 직전 | 경고 후 계속 |
...
- `pre_commit` 이 **경고 후 계속**인 이유: 포맷은 정확성 게이트가 아니고, 커밋 이전의
  ...
  세우지 않는다.
- **이름과 위치가 정확히 겹치지 않는다는 점을 알고 쓸 것.** `project-start` 는 커밋하지
  않는다 — 소스 커밋은 `project-done` Step 5 다. Step 7 은 "구현이 끝난 시점" 이고
  그 사이에 리뷰(§8)와 그 수정이 들어간다. 그래서 `project-done` 을 단독 호출하는
  경로(그 스킬의 트리거에 명시돼 있다)에서도 커밋 직전에 한 번 더 돈다. 같은 명령이
  두 번 도는 것은 포매터라면 무해하다 — **멱등이 아닌 명령을 이 훅에 넣지 말 것.**
- `project-adr` 이 만드는 ADR 커밋은 이 훅의 대상이 아니다. 문서 커밋이고, 포맷 대상
  소스가 아직 없을 수도 있다.
