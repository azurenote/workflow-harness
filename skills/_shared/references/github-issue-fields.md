...
**"실패" 는 호출이 성립하지 않은 경우다.** `create-issue` 가 2(환경 거부 — 보드를 못 읽어
라벨을 판정할 수 없다)나 4(생성 여부 미상)로 끝난 것은 판정이 **수행된** 결과이지 호출의
실패가 아니다. 이 폴백에는 예약 라벨 판정이 아예 없으므로, 그 두 코드를 실패로 읽고 내려오면
