...
| `project-iterate` | plan → issue → start → done 을 한 번에 실행. 플랜 승인 뒤로는 `## Questions After Plan Approval` 의 조건에서만 묻는다. 기본은 워크트리에서 분기하고, `in-place` 를 붙이면 main checkout 에서 제자리 분기한다. `project-iterate <id>` 는 기존 이슈에서 출발하며, 플랜이 없으면 이슈 본문으로 쓰고 연결 모드로 붙인다 |
